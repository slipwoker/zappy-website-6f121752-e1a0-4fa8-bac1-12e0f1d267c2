...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (contactForm) {
        ...
        contactForm.addEventListener('input', function(e) {
            ...
            if (input.closest && (input.closest('.form-group') || input.name)) {
                clearError(input);
            }
        });
    }
...
/* ZAPPY_GSAP_MOTION_RUNTIME (Design Personality Engine) */
(function(){
  if (typeof window === 'undefined' || typeof document === 'undefined') return;
  if (window.__zappyGsapRuntimeInit) return;
  window.__zappyGsapRuntimeInit = true;

  // CDN coordinates (kept in sync with GSAP_CDN_TAGS). The runtime SELF-BOOTSTRAPS
  // GSAP from here whenever window.gsap is missing. The head <script> tags from
  // buildGsapHead() are only a fast-path: the preview/editor renderer
  // (previewHtmlService) and some publish paths rebuild the document <head> from
  // scratch and drop external <script src> tags, so relying on the head alone
  // left every served site un-animated (content visible but frozen, count-ups
  // stuck at 0). This loader runs from /script.js — which is always served
  // verbatim — so motion works identically on preview, editor, and published.
  var GSAP_VERSION = '3.13.0';
  var GSAP_CDN_BASE = 'https://cdnjs.cloudflare.com/ajax/libs/gsap/' + GSAP_VERSION;
  var SIGNATURE_PLUGINS_ENABLED = true;
  var CINEMATIC_HERO_ENABLED = false;

  var docEl = document.documentElement;
  var isRTL = (document.dir === 'rtl') || (docEl.getAttribute('dir') === 'rtl');
  var dirSign = isRTL ? -1 : 1;
  var reduce = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  function revealAllStatically(){
    docEl.classList.add('gsap-motion-reduced');
    // Make sure nothing authored as initially-hidden stays invisible.
    var hidden = document.querySelectorAll('[data-reveal],[data-reveal-stagger] > *,[data-clip-reveal],[data-split-text],[data-scramble],[data-cinematic-hero] [data-hero-layer],[data-cinematic-hero] [data-hero-visual]');
    for (var i = 0; i < hidden.length; i++) {
      hidden[i].style.opacity = '1';
      hidden[i].style.transform = 'none';
      hidden[i].style.clipPath = 'none';
    }
    document.querySelectorAll('[data-draw-svg]').forEach(function(el){
  ...
  // Inject one CDN <script> (ordered, non-async), resolve via callback. Idempotent:
  // re-uses an in-flight/loaded tag (head fast-path tag OR a prior self-load).
  function loadScript(src, cb){
    var existing = document.querySelector('script[data-zappy-gsap="' + src + '"]');
    if (existing) {
      if (existing.getAttribute('data-zappy-loaded') === '1') { cb(); return; }
      existing.addEventListener('load', function(){ cb(); });
      existing.addEventListener('error', function(){ cb(new Error('load failed: ' + src)); });
      return;
    }
    var s = document.createElement('script');
    s.src = src;
    s.async = false; // preserve gsap-before-ScrollTrigger execution order
    s.setAttribute('data-zappy-gsap', src);
    s.onload = function(){ s.setAttribute('data-zappy-loaded', '1'); cb(); };
    s.onerror = function(){ cb(new Error('load failed: ' + src)); };
    (document.head || docEl).appendChild(s);
  }

  // Ensure window.gsap + window.ScrollTrigger exist (from head tags OR self-load).
  function ensureGsap(cb){
    if (window.gsap && window.ScrollTrigger) { cb(); return; }
    loadScript(GSAP_CDN_BASE + '/gsap.min.js', function(err){
      if (err || !window.gsap) { cb(err || new Error('gsap unavailable')); return; }
      loadScript(GSAP_CDN_BASE + '/ScrollTrigger.min.js', function(err2){
        if (err2 || !window.ScrollTrigger) { cb(err2 || new Error('ScrollTrigger unavailable')); return; }
        cb();
      });
    });
  }

  function loadPlugin(file, globalName, cb){
    if (window[globalName]) { cb(); return; }
    loadScript(GSAP_CDN_BASE + '/' + file, function(err){
      if (err || !window[globalName]) { cb(err || new Error(globalName + ' unavailable')); return; }
      cb();
    });
  }

  function setupLegacySplitText(gsap, ScrollTrigger){
    Array.prototype.slice.call(document.querySelectorAll('[data-split-text]')).forEach(function(el){
      if (el.__zappySplit) return;
      el.__zappySplit = true;
      var mode = (el.getAttribute('data-split-text') || 'words').toLowerCase();
  ...
  function setupCinematicHero(gsap, ScrollTrigger, q){
    if (!CINEMATIC_HERO_ENABLED) return;
    var heroes = q('[data-cinematic-hero]');
    if (!heroes.length) return;
    var finePointer = !window.matchMedia || window.matchMedia('(pointer: fine)').matches;
    heroes.forEach(function(hero){
      if (hero.__zappyCinematicHero) return;
      // Cinematic Scroll Story owns the ONE ScrollTrigger on its host — the
      // legacy cinematic-hero entrance/pin stack must never double-drive the
      // same section (double pin / competing transforms class of bugs).
      if (hero.hasAttribute('data-zappy-cinematic-story') || hero.closest('[data-zappy-cinematic-story]')) return;
      hero.__zappyCinematicHero = true;

      var layers = Array.prototype.slice.call(hero.querySelectorAll('[data-depth],[data-hero-layer]'));
      var visual = hero.querySelector('[data-hero-visual], [data-tilt-card], .hero-visual, [class*="hero-visual"], [class*="mockup"], [class*="image-wrap"], picture, img, video');
      var textGroup = hero.querySelector('[data-hero-layer="text"], .hero-content, [class*="hero-content"], [class*="copy"], [class*="text"]') || hero;
      var supporting = Array.prototype.slice.call(textGroup.querySelectorAll('p, .subtitle, .eyebrow, .kicker, .btn, .button, a[class*="btn"], button')).filter(function(el){
        return !el.closest('[data-split-text]');
      }).slice(0, 6);

      if (visual) {
        visual.setAttribute('data-hero-visual', visual.getAttribute('data-hero-visual') || '');
        if (!visual.hasAttribute('data-depth')) visual.setAttribute('data-depth', '0.45');
      }

      var heroRect = hero.getBoundingClientRect();
      var heroStartsAboveFold = heroRect.top < (window.innerHeight || document.documentElement.clientHeight || 0) * 0.85;
      var entrance = gsap.timeline({
        defaults: { duration: 0.9, ease: 'power3.out' },
        delay: heroStartsAboveFold ? 0.08 : 0,
        scrollTrigger: heroStartsAboveFold ? null : { trigger: hero, start: 'top 88%', toggleActions: 'play none none none' }
      });
      if (supporting.length) entrance.from(supporting, { y: 26, opacity: 0, stagger: 0.08, clearProps: 'transform,opacity' }, 0.12);
      if (visual) entrance.from(visual, { y: 44, z: -80, rotateX: 8, rotateY: -10 * dirSign, scale: 0.92, opacity: 0, transformPerspective: 900, transformOrigin: '50% 55%', clearProps: 'transform,opacity' }, 0.08);
      if (layers.length) entrance.from(layers.filter(function(el){ return el !== visual && !supporting.includes(el); }), { y: 18, opacity: 0, stagger: 0.04, clearProps: 'opacity' }, 0.18);

      if (hero.hasAttribute('data-scroll-scene')) {
        var camera = hero.querySelector('[data-hero-camera]') || hero;
        var sceneMode = (hero.getAttribute('data-scroll-scene') || 'scrub').toLowerCase();
        var requestedPin = sceneMode === 'pin' || sceneMode === 'pinned';
        var canPinHero = function(){
          var viewportH = window.innerHeight || document.documentElement.clientHeight || 0;
          var heroH = hero.offsetHeight || hero.getBoundingClientRect().height || 0;
          var coarsePointer = window.matchMedia && window.matchMedia('(pointer: coarse)').matches;
          // Tall heroes already exceed the viewport. Pinning them inserts a spacer
          // after the visible content has scrolled away, producing a blank "stuck"
          // gap before the next section. Keep cinematic depth, but don't pin.
          return viewportH >= 640 && !coarsePointer && heroH <= viewportH * 1.1;
        };
        var shouldPin = requestedPin && canPinHero();
        var scrub = gsap.timeline({
          scrollTrigger: shouldPin ? {
            trigger: hero,
            start: 'top top',
            end: function(){ return '+=' + Math.round((window.innerHeight || 0) * 0.65); },
            scrub: true,
            pin: true,
            pinSpacing: true,
            anticipatePin: 1,
        ...
        scrub.to(camera, { scale: 0.96, ease: 'none' }, 0);
        if (visual) scrub.to(visual, { y: -36, rotateX: -3, rotateY: 6 * dirSign, scale: 1.04, ease: 'none' }, 0);
        layers.forEach(function(layer){
          var depth = Math.min(Math.max(parseFloat(layer.getAttribute('data-depth')) || 0.2, 0), 1);
          scrub.to(layer, { y: -80 * depth, x: 22 * depth * dirSign, ease: 'none' }, 0);
        });
      }

      q('[data-orbit]').filter(function(el){ return hero.contains(el); }).forEach(function(el, index){
        var amount = parseFloat(el.getAttribute('data-orbit')) || (8 + index * 2);
        gsap.to(el, {
          x: amount * dirSign,
          y: -amount,
          rotate: index % 2 ? -4 : 4,
      ...
      if (finePointer) {
        var tiltTargets = Array.prototype.slice.call(hero.querySelectorAll('[data-tilt-card],[data-hero-visual]'));
        var depthTargets = layers.filter(function(el){ return !tiltTargets.includes(el); });
        hero.addEventListener('mousemove', function(e){
          var rect = hero.getBoundingClientRect();
          var nx = ((e.clientX - rect.left) / Math.max(rect.width, 1) - 0.5) * 2;
          var ny = ((e.clientY - rect.top) / Math.max(rect.height, 1) - 0.5) * 2;
          hero.style.setProperty('--zappy-glow-x', ((nx + 1) * 50).toFixed(2) + '%');
          hero.style.setProperty('--zappy-glow-y', ((ny + 1) * 50).toFixed(2) + '%');
          tiltTargets.forEach(function(el){
            gsap.to(el, { rotateY: nx * 7 * dirSign, rotateX: -ny * 6, x: nx * 8, y: ny * 6, duration: 0.45, ease: 'power3.out', overwrite: true });
          });
          depthTargets.forEach(function(el){
            var depth = Math.min(Math.max(parseFloat(el.getAttribute('data-depth')) || 0.2, 0), 1);
            gsap.to(el, { x: nx * depth * 18 * dirSign, y: ny * depth * 14, duration: 0.6, ease: 'power3.out', overwrite: true });
          });
        });
        hero.addEventListener('mouseleave', function(){
          tiltTargets.concat(depthTargets).forEach(function(el){
            gsap.to(el, { rotateY: 0, rotateX: 0, x: 0, y: 0, duration: 0.7, ease: 'power3.out', overwrite: true });
          });
        });
      }
    });
  ...
  if (reduce) { ready(revealAllStatically); return; }

  ready(function(){
    ensureGsap(function(err){
      if (err) {
        // CDN unreachable — never leave content hidden.
        revealAllStatically();
        return;
      }
      initMotion();
  ...
  function initMotion(){
    ...
    docEl.classList.add('gsap-motion-enabled');

    var q = function(sel){ return Array.prototype.slice.call(document.querySelectorAll(sel)); };

    // ---------- data-reveal ------------------------------------------------
    q('[data-reveal]').forEach(function(el){
      if (el.closest('[data-reveal-stagger]')) return; // handled by parent
      var kind = (el.getAttribute('data-reveal') || 'up').toLowerCase();
      var from = { opacity: 0 };
      var dist = 28;
      if (kind === 'up') from.y = dist;
      else if (kind === 'left') from.x = dist * dirSign;        // RTL-aware
      else if (kind === 'right') from.x = -dist * dirSign;      // RTL-aware
      else if (kind === 'scale') { from.scale = 0.92; from.y = 12; }
      // 'fade' = opacity only
      gsap.from(el, {
        scrollTrigger: { trigger: el, start: 'top 85%', toggleActions: 'play none none none' },
        duration: 0.7, ease: 'power3.out', clearProps: 'all', ...from
      });
    });

    // ---------- data-reveal-stagger ---------------------------------------
    q('[data-reveal-stagger]').forEach(function(parent){
      var children = Array.prototype.slice.call(parent.children);
      if (children.length === 0) return;
      var step = parseFloat(getComputedStyle(parent).getPropertyValue('--stagger-step')) / 1000 || 0.08;
      gsap.from(children, {
        scrollTrigger: { trigger: parent, start: 'top 82%', toggleActions: 'play none none none' },
        opacity: 0, y: 24, duration: 0.6, ease: 'power3.out', stagger: step, clearProps: 'all'
      });
    });
    ...
    q('[data-parallax]').forEach(function(el){
      ...
      var distance = Math.min(Math.max(speed, 0.05), 0.5) * 160;
    ...
    // ---------- data-count-up ---------------------------------------------
    // Large values (e.g. 1,000,000) at the display-xl stat font overflow their
    // grid cell and overlap neighbours, and a WORD suffix (" Days") rendered at
    // that giant font wraps and corrupts the grid. Compact-format big numbers
    // (1000000 -> "1M", 12000 -> "12K") and render word suffixes in a small
    // sibling span. Symbol suffixes (+, %, x) stay inline at full size.
    function zappyFmtCount(v, target, decimals){
      var a = Math.abs(target);
      if (a >= 1000000) { return String(Math.round((v / 1000000) * 10) / 10).replace(/\.0$/, '') + 'M'; }
      if (a >= 10000) { return Math.round(v / 1000) + 'K'; }
      if (decimals > 0) return v.toFixed(decimals);
      try { return Math.round(v).toLocaleString(); } catch (e) { return String(Math.round(v)); }
    }
    function zappyEnsureCountSuffixStyle(){
      if (document.getElementById('zappy-count-suffix-style')) return;
      var s = document.createElement('style');
      s.id = 'zappy-count-suffix-style';
      s.textContent = '.zappy-count-suffix{font-size:0.42em;font-weight:inherit;opacity:0.8;margin-inline-start:0.12em;letter-spacing:0;white-space:nowrap;line-height:1;}';
      (document.head || document.documentElement).appendChild(s);
    }
    q('[data-count-up]').forEach(function(el){
      var target = parseFloat(el.getAttribute('data-count-up')) || parseFloat((el.textContent || '').replace(/[^0-9.\-]/g, '')) || 0;
      var duration = (parseInt(el.getAttribute('data-count-duration'), 10) || 1600) / 1000;
      var prefix = el.getAttribute('data-count-prefix') || '';
      var suffix = el.getAttribute('data-count-suffix') || '';
      var decimals = parseInt(el.getAttribute('data-count-decimals'), 10) || 0;
      var suffixIsWord = !!suffix && suffix.trim().length >= 2 && /[A-Za-z\u0590-\u05FF]/.test(suffix);
      var numNode = null;
      if (suffixIsWord) {
        zappyEnsureCountSuffixStyle();
        el.textContent = '';
        numNode = document.createTextNode('');
        // Seed the formatted start value so the number is visible BEFORE the
        // stat scrolls into view — otherwise the user sees the word suffix
        // ("Days") alone with no count until ScrollTrigger fires.
        numNode.nodeValue = prefix + zappyFmtCount(0, target, decimals);
        el.appendChild(numNode);
        var sufEl = document.createElement('span');
        sufEl.className = 'zappy-count-suffix';
        sufEl.textContent = suffix.trim();
      ...
      gsap.to(obj, {
        ...
        scrollTrigger: { trigger: el, start: 'top 85%', toggleActions: 'play none none none' },
        onUpdate: function(){
          var num = prefix + zappyFmtCount(obj.v, target, decimals);
          if (suffixIsWord) { numNode.nodeValue = num; }
    ...
    // ---------- data-split-text -------------------------------------------
    if (q('[data-split-text]').length && !SIGNATURE_PLUGINS_ENABLED) {
      setupLegacySplitText(gsap, ScrollTrigger);
    } else if (SIGNATURE_PLUGINS_ENABLED && q('[data-split-text]').length) {
      loadPlugin('SplitText.min.js', 'SplitText', function(err){
        if (err) { setupLegacySplitText(gsap, ScrollTrigger); return; }
        gsap.registerPlugin(window.SplitText);
        var run = function(){
          q('[data-split-text]').forEach(function(el){
            if (el.__zappySplit) return;
            el.__zappySplit = true;
            if (el.querySelector('a, button, input, textarea, select')) return;
            var mode = (el.getAttribute('data-split-text') || 'words').toLowerCase();
            if (['lines', 'words', 'chars'].indexOf(mode) === -1) mode = 'words';
            var mask = el.hasAttribute('data-split-mask') || mode === 'lines';
            var split = window.SplitText.create(el, { type: mode, mask: mask ? mode : false, autoSplit: true });
            var targets = split[mode] || split.words || split.chars || [];
            if (!targets.length) return;
            gsap.from(targets, {
              scrollTrigger: { trigger: el, start: 'top 88%', toggleActions: 'play none none none' },
              yPercent: 110,
              opacity: 0,
              duration: mode === 'chars' ? 0.55 : 0.8,
              ease: 'power4.out',
              stagger: mode === 'chars' ? 0.018 : (mode === 'words' ? 0.045 : 0.09)
            });
          });
          ScrollTrigger.refresh();
        };
    ...
    // ---------- data-draw-svg ---------------------------------------------
    if (SIGNATURE_PLUGINS_ENABLED && q('[data-draw-svg]').length) {
      loadPlugin('DrawSVGPlugin.min.js', 'DrawSVGPlugin', function(err){
        if (err) return;
        gsap.registerPlugin(window.DrawSVGPlugin);
        q('[data-draw-svg]').forEach(function(el){
          var targets = el.matches('path,line,polyline,polygon,circle,ellipse,rect')
            ? [el]
            : Array.prototype.slice.call(el.querySelectorAll('path,line,polyline,polygon,circle,ellipse,rect'));
          targets.forEach(function(path){
            if (path.__zappyDrawSvg) return;
            path.__zappyDrawSvg = true;
            var duration = parseFloat(el.getAttribute('data-draw-duration') || path.getAttribute('data-draw-duration')) || 1.2;
            gsap.from(path, {
              drawSVG: '0%',
              duration: duration,
              ease: 'power2.out',
    ...
    // ---------- data-scramble ---------------------------------------------
    if (SIGNATURE_PLUGINS_ENABLED && q('[data-scramble]').length) {
      loadPlugin('ScrambleTextPlugin.min.js', 'ScrambleTextPlugin', function(err){
        if (err) return;
        gsap.registerPlugin(window.ScrambleTextPlugin);
        q('[data-scramble]').forEach(function(el){
          if (el.__zappyScramble) return;
          el.__zappyScramble = true;
          var finalText = el.getAttribute('data-scramble-text') || el.textContent || '';
          var chars = el.getAttribute('data-scramble-chars') || 'upperCase';
          gsap.to(el, {
            duration: 1,
            ease: 'none',
            scrambleText: { text: finalText, chars: chars, revealDelay: 0.2, speed: 0.35 },
            scrollTrigger: { trigger: el, start: 'top 88%', toggleActions: 'play none none none' }
          });
        });
      });
    ...
    // ---------- data-morph -------------------------------------------------
    if (SIGNATURE_PLUGINS_ENABLED && q('[data-morph]').length) {
      loadPlugin('MorphSVGPlugin.min.js', 'MorphSVGPlugin', function(err){
        if (err) return;
        gsap.registerPlugin(window.MorphSVGPlugin);
        q('[data-morph]').forEach(function(el){
          if (el.__zappyMorph) return;
          el.__zappyMorph = true;
          var target = el.getAttribute('data-morph-to') || el.getAttribute('data-morph');
          if (!target || target === 'true') return;
          var loop = el.hasAttribute('data-morph-loop');
          gsap.to(el, {
            morphSVG: target,
            duration: 1.4,
            ease: 'power2.inOut',
            repeat: loop ? -1 : 0,
            yoyo: loop,
            scrollTrigger: loop ? undefined : { trigger: el, start: 'top 85%', toggleActions: 'play none none reverse' }
          });
        });
      });
    }
    ...
    // ---------- data-clip-reveal ------------------------------------------
    q('[data-clip-reveal]').forEach(function(el){
    ...
    // ---------- data-marquee ----------------------------------------------
    q('[data-marquee]').forEach(function(el){
      var tracks = el.querySelectorAll('.marquee-track');
      if (tracks.length === 0) return;
      for (var t = 1; t < tracks.length; t++) { tracks[t].parentNode.removeChild(tracks[t]); }
      var track = tracks[0];
      if (track.children.length === 0) return;
      var speed = parseFloat(el.getAttribute('data-marquee-speed') || el.getAttribute('data-marquee')) || 40;
      var clone = track.cloneNode(true);
      clone.setAttribute('aria-hidden', 'true');
      el.appendChild(clone);
      var dir = isRTL ? '100%' : '-100%';
      gsap.set([track, clone], { display: 'inline-flex' });
      gsap.to([track, clone], { xPercent: isRTL ? 100 : -100, repeat: -1, duration: speed, ease: 'none' });
    });
    ...
    // ---------- data-pin / data-pinned ------------------------------------
    q('[data-pin],[data-pinned]').forEach(function(el){
    ...
    q('[data-horizontal-scroll]').forEach(function(section){
      var track = section.querySelector('.horizontal-track') || section.firstElementChild;
      if (!track) return;
    ...
    q('[data-scroll-progress]').forEach(function(bar){
      gsap.set(bar, { transformOrigin: isRTL ? 'right center' : 'left center', scaleX: 0 });
      gsap.to(bar, {
        scaleX: 1, ease: 'none',
    ...
    // ---------- data-cinematic-hero ---------------------------------------
    setupCinematicHero(gsap, ScrollTrigger, q);

    // Recalculate after fonts/images settle.
    window.addEventListener('load', function(){ ScrollTrigger.refresh(); });
  }
})();
/* End ZAPPY_GSAP_MOTION_RUNTIME */



/* ZAPPY_CINEMATIC_STORY_RUNTIME (Cinematic Scroll Story Engine) */
(function(){
  if (typeof window === 'undefined' || typeof document === 'undefined') return;
  if (window.__zappyCinematicStoryInit) return;
  window.__zappyCinematicStoryInit = true;

  var STORY_ENABLED = true;
  var MODEL_VIEWER_SRC = 'https://cdn.jsdelivr.net/npm/@google/model-viewer@4.1.0/dist/model-viewer.min.js';
  var GSAP_CDN_BASE = 'https://cdnjs.cloudflare.com/ajax/libs/gsap/3.13.0';
  var STORY_PAGE_MAX_BEATS = 5;
  // A single product should feel art-directed across the page, not like one
  // centered turntable stretched for thousands of pixels. Each real homepage
  // section gets a deterministic stage pose; adjacent poses are interpolated.
  var PAGE_STAGE_POSES = [
    { xVw: 18, yVh: 0, scale: 1.00, opacity: 1.00 },
    { xVw: -27, yVh: -2, scale: 0.62, opacity: 0.92 },
    { xVw: 27, yVh: 2, scale: 0.68, opacity: 0.96 },
    { xVw: -24, yVh: 0, scale: 0.58, opacity: 0.90 },
    { xVw: 20, yVh: -1, scale: 0.64, opacity: 0.94 }
  ];
  ...
  function track(event, data){
    try { if (window.zappyAnalytics && window.zappyAnalytics.track) window.zappyAnalytics.track(event, data || {}); } catch (e) {}
  }

  // Editor iframe (non-fullscreen preview) → static, controllable stage so
  // section selection tools stay usable. Fullscreen preview + published sites
  // run the full sequence.
  function isEditorPreview(){
    try {
      return location.pathname.indexOf('/api/website/preview/') === 0
        && location.pathname.indexOf('preview-fullscreen') === -1;
  ...
  function parseManifest(host){
    var script = host.querySelector('script[data-zappy-cinematic-story-manifest]');
    if (!script) return null;
    try {
      var m = JSON.parse(script.textContent);
      if (!m || m.version !== 1 || !m.renderers) return null;
      return m;
    } catch (e) { return null; }
  }

  function isPageBeatCandidate(el){
    if (!el || el.nodeType !== 1 || String(el.tagName || '').toLowerCase() !== 'section') return false;
    // A second pinned/horizontal mechanic inside the corridor would compete
    // with the story's one scroll driver. Stop the corridor before it.
    if (el.hasAttribute('data-horizontal-scroll') || el.hasAttribute('data-pin')) return false;
    if (el.querySelector('[data-horizontal-scroll], [data-pin]')) return false;
    return true;
  }

  /**
   * Progressively upgrade the stored v1 hero scaffold into a page corridor.
   * This runs only after resolveTier() selected model/frames, so reduced-motion,
   * save-data, editor-preview, and static visitors keep the original DOM and
   * normal section flow byte-for-byte.
   *
   * The wrapper is runtime-only: existing content versions/manifests stay
   * compatible, while new and already-published v1 sites gain the richer
   * multi-section experience as soon as their marked runtime is refreshed.
   */
  function promoteHeroStoryToPage(heroHost){
    if (!heroHost) return heroHost;
    if (heroHost.getAttribute('data-story-scope') === 'page') return heroHost;
    if (String(heroHost.tagName || '').toLowerCase() !== 'section') return heroHost;

    var parent = heroHost.parentElement;
    var main = heroHost.closest ? heroHost.closest('main') : null;
    if (!parent || !main || !main.contains(heroHost)) return heroHost;

    var beats = [];
    var cursor = heroHost;
    while (cursor && beats.length < STORY_PAGE_MAX_BEATS) {
      if (!isPageBeatCandidate(cursor)) break;
      beats.push(cursor);
      cursor = cursor.nextElementSibling;
    }
    if (beats.length < 2) return heroHost;

    var corridor = document.createElement('div');
    corridor.className = 'zappy-story-page-corridor';
    corridor.setAttribute('data-zappy-cinematic-story', heroHost.getAttribute('data-zappy-cinematic-story') || 'story');
    corridor.setAttribute('data-story-state', heroHost.getAttribute('data-story-state') || 'ready');
    corridor.setAttribute('data-story-scope', 'page');
    corridor.setAttribute('data-story-tier', heroHost.getAttribute('data-story-tier') || 'frames');
    var label = heroHost.getAttribute('data-story-label');
    var ariaLabel = heroHost.getAttribute('aria-label');
    if (label) corridor.setAttribute('data-story-label', label);
    if (ariaLabel) corridor.setAttribute('aria-label', ariaLabel);

    parent.insertBefore(corridor, heroHost);
    beats.forEach(function(beat, index){
      beat.setAttribute('data-story-page-beat', '');
      beat.setAttribute('data-story-page-beat-index', String(index));
      if (index === 0) beat.setAttribute('data-story-page-beat-kind', 'hero');
      corridor.appendChild(beat);
    });

    // The manifest belongs to the coordinator host. Moving the existing node
    // preserves its exact escaped payload and keeps parse/debug tooling useful.
    var script = heroHost.querySelector('script[data-zappy-cinematic-story-manifest]');
    if (script) corridor.insertBefore(script, corridor.firstChild);

    heroHost.removeAttribute('data-zappy-cinematic-story');
    heroHost.removeAttribute('data-story-state');
    heroHost.removeAttribute('data-story-tier');
    heroHost.removeAttribute('data-story-label');
    corridor.__zappyStoryInit = true;
    return corridor;
  }

  function resolveTier(manifest){
    if (reduce) return { tier: 'static', reason: 'reduced-motion' };
    if (navigator.connection && navigator.connection.saveData) return { tier: 'static', reason: 'save-data' };
    if (isEditorPreview()) return { tier: 'static', reason: 'editor-preview' };
    var allowModel = !!(manifest.renderers.allowModel && manifest.model && manifest.model.glbUrl);
    var allowFrames = !!(manifest.renderers.allowFrames && manifest.frames && manifest.frames.desktop && manifest.frames.mobile);
    if (!allowModel && !allowFrames) return { tier: 'static', reason: 'no-renderer' };
    ...
    if (coarse || small || lowMem) {
      return allowFrames ? { tier: 'frames', variant: 'mobile', reason: 'mobile-device' }
                         : { tier: 'static', reason: 'mobile-no-frames' };
    }
    if (allowModel && webglAvailable()) return { tier: 'model', reason: 'capable-desktop' };
    if (allowFrames) return { tier: 'frames', variant: 'desktop', reason: allowModel ? 'webgl-unavailable' : 'frames-preferred' };
    return { tier: 'static', reason: 'no-capable-renderer' };
  }
  ...
  function frameUrl(set, i){
    var idx = Math.max(0, Math.min(set.count - 1, Math.round(i)));
    var padded = String(idx);
    while (padded.length < 3) padded = '0' + padded;
    return set.baseUrl + '/f' + padded + '.webp';
  }

  // ── Frame adapter ─────────────────────────────────────────────────────────
  function createFrameAdapter(host, stage, set, label){
    var canvas = document.createElement('canvas');
    canvas.className = 'zappy-story-canvas';
    ...
    var bitmaps = {};          // index -> ImageBitmap | HTMLImageElement
    var order = [];            // LRU insertion order
    var MAX_CACHE = 12;        // decoded frames resident at once
    var inflight = {};
    var concurrent = 0;
    var MAX_CONCURRENT = 3;
    var currentFrame = 0;      // float target
    var drawnFrame = -1;
    var lastDirection = 1;
    var paused = false;
    var rafId = null;
    var supportsBitmap = typeof createImageBitmap === 'function';
    var corsBroken = false;    // once a cors fetch fails, use <img> directly
    ...
    function load(index){
      var i = Math.max(0, Math.min(set.count - 1, Math.round(index)));
      if (bitmaps[i] || inflight[i] || concurrent >= MAX_CONCURRENT) return;
      inflight[i] = true;
      concurrent++;
      var url = frameUrl(set, i);
      ...
      // NOTE: the <img> fallback deliberately does NOT set crossOrigin.
      // Frames are only ever drawImage()d — never read back — so a tainted
      // canvas is harmless, and a crossOrigin request would hard-fail when
      // the asset host (R2 custom domain) lacks a CORS policy. After the
      // first cors fetch failure we go straight to <img> for every frame
      // so we don't pay a doomed request per frame.
      function loadViaImg(){
        var img = new Image();
      ...
      if (!corsBroken && supportsBitmap && typeof fetch === 'function') {
        fetch(url, { mode: 'cors' })
    ...
    function nearestLoaded(target){
      ...
      for (var d = 1; d < set.count; d++) {
        if (bitmaps[target - d]) return target - d;
        if (bitmaps[target + d]) return target + d;
      }
      return -1;
    }
    ...
    function draw(){
      rafId = null;
      var target = Math.round(currentFrame);
      var candidate = nearestLoaded(target);
      if (candidate === -1 || candidate === drawnFrame) return;
      drawnFrame = candidate;
      var bmp = bitmaps[candidate];
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      try {
        ctx.drawImage(bmp, 0, 0, canvas.width, canvas.height);
        // Frames are transparent cutouts, so the poster underneath would show
        // through as a ghost double-subject. Hide it once real frames paint.
        host.setAttribute('data-story-frames-live', 'true');
      } catch (e) {}
    }
    ...
    function prefetch(target, direction){
      // Directional window: 2 ahead, 1 behind (flipped when scrolling up).
    ...
    // Warm the keyframes once the story approaches the viewport.
    var warmed = false;
    function warmup(){
      if (warmed) return;
      warmed = true;
      load(0);
      load(Math.floor((set.count - 1) / 2));
      load(set.count - 1);
    }
    if ('IntersectionObserver' in window) {
      var io = new IntersectionObserver(function(entries){
        for (var k = 0; k < entries.length; k++) {
          if (entries[k].isIntersecting) { warmup(); io.disconnect(); break; }
        }
      }, { rootMargin: '50% 0px' });
      io.observe(host);
    } else {
    ...
    return {
      ...
      onProgress: function(progress){
        var next = progress * (set.count - 1);
        lastDirection = next >= currentFrame ? 1 : -1;
        currentFrame = next;
        prefetch(Math.round(next), lastDirection);
        scheduleDraw();
      },
      firstFrameLoaded: function(){ return !!bitmaps[0]; }
    };
  }

  // ── Model adapter ─────────────────────────────────────────────────────────
  function loadModelViewer(cb){
    if (window.customElements && window.customElements.get && window.customElements.get('model-viewer')) { cb(); return; }
    var existing = document.querySelector('script[data-zappy-model-viewer="' + MODEL_VIEWER_SRC + '"]');
    if (existing) {
      existing.addEventListener('load', function(){ cb(); });
      existing.addEventListener('error', function(){ cb(new Error('model-viewer failed to load')); });
      if (existing.getAttribute('data-zappy-loaded') === '1') cb();
      return;
    }
    var s = document.createElement('script');
    s.type = 'module';
    s.src = MODEL_VIEWER_SRC;
    s.setAttribute('data-zappy-model-viewer', MODEL_VIEWER_SRC);
    s.onload = function(){ s.setAttribute('data-zappy-loaded', '1'); cb(); };
    s.onerror = function(){ cb(new Error('model-viewer failed to load')); };
    (document.head || docEl).appendChild(s);
  }

  function lerp(a, b, t){ return a + (b - a) * t; }

  function createModelAdapter(host, stage, model, label, onFail){
    var cam = model.camera || {};
    var dirSign = isRTL ? -1 : 1;
    var viewer = document.createElement('model-viewer');
    viewer.className = 'zappy-story-viewer';
    ...
    // NO auto-rotate: scroll owns the camera (plan contract).
    viewer.setAttribute('aria-label', label || 'Interactive 3D product story');
    ...
    viewer.addEventListener('error', function(){
      if (failed) return;
      failed = true;
      try { stage.removeChild(viewer); } catch (e) {}
      onFail('model-error');
    });
    viewer.addEventListener('load', function(){
      host.setAttribute('data-story-model-loaded', 'true');
    });

    return {
      kind: 'model',
      onProgress: function(progress){
        if (failed || !viewer.jumpCameraToGoal) return;
        var theta = lerp(cam.thetaStartDeg || 0, cam.thetaEndDeg || 360, progress) * dirSign;
        var phi = lerp(cam.phiStartDeg || 75, cam.phiEndDeg || 75, progress);
        var radius = lerp(cam.radiusStartPct || 105, cam.radiusEndPct || 105, progress);
        var fov = lerp(cam.fovStartDeg || 32, cam.fovEndDeg || 32, progress);
        viewer.cameraOrbit = theta.toFixed(2) + 'deg ' + phi.toFixed(2) + 'deg ' + radius.toFixed(2) + '%';
        viewer.fieldOfView = fov.toFixed(2) + 'deg';
        try { viewer.jumpCameraToGoal(); } catch (e) {}
      }
    };
  }

  // ── Chapters choreography ─────────────────────────────────────────────────
  function setupChapters(host, gsap){
    var chapters = Array.prototype.slice.call(host.querySelectorAll('[data-story-chapter]'));
    if (!chapters.length) return function(){};
    var n = chapters.length;
    chapters.forEach(function(ch, i){
      if (i > 0) gsap.set(ch, { autoAlpha: 0, y: 24 });
      ch.setAttribute('data-story-chapter-index', String(i));
    });
    return function(progress){
      // Each chapter owns an equal progress band with a soft crossfade edge.
      var band = 1 / n;
      for (var i = 0; i < n; i++) {
        var start = i * band;
        var end = start + band;
        var fadeZone = band * 0.25;
        var alpha;
        if (progress < start - fadeZone || progress > end + fadeZone) alpha = 0;
        else if (progress < start) alpha = (progress - (start - fadeZone)) / fadeZone;
        else if (progress > end) alpha = 1 - (progress - end) / fadeZone;
        else alpha = 1;
        if (i === 0 && progress <= start) alpha = 1;                 // hero copy visible pre-scroll
        if (i === n - 1 && progress >= end) alpha = 1;               // last beat holds
        gsap.set(chapters[i], { autoAlpha: alpha, y: (1 - alpha) * 24 });
      }
    };
  }

  // ── Page-corridor choreography ─────────────────────────────────────────────
  function smoothstep(t){
    var x = Math.max(0, Math.min(1, Number(t) || 0));
    return x * x * (3 - 2 * x);
  }

  function setupPageStory(host, stage, gsap, sceneId){
    var beats = Array.prototype.slice.call(host.querySelectorAll('[data-story-page-beat]'));
    if (beats.length < 2) return setupChapters(host, gsap);

    // Existing v1 heroes may carry 3-4 micro-copy beats. Keep them, but spend
    // only the first page-section band on them; the remaining model/frame
    // sequence now belongs to the real following sections.
    var applyHeroChapters = setupChapters(beats[0], gsap);
    var activeBeat = -1;
    stage.style.setProperty('transform-origin', '50% 50%');
    stage.style.setProperty('will-change', 'transform, opacity');

    return function(progress){
      var maxIndex = beats.length - 1;
      var position = Math.max(0, Math.min(maxIndex, progress * maxIndex));
      var fromIndex = Math.floor(position);
      var toIndex = Math.min(maxIndex, fromIndex + 1);
      var mix = smoothstep(position - fromIndex);
      var fromPose = PAGE_STAGE_POSES[fromIndex % PAGE_STAGE_POSES.length];
      var toPose = PAGE_STAGE_POSES[toIndex % PAGE_STAGE_POSES.length];
      var dir = isRTL ? -1 : 1;
      var compact = window.matchMedia && window.matchMedia('(max-width: 768px)').matches;
      // Mobile stacks copy below the compact visual, so keep the subject nearer
      // center and slightly larger; desktop has room for the dramatic alternating
      // side compositions.
      var xVw = lerp(fromPose.xVw, toPose.xVw, mix) * dir * (compact ? 0.55 : 1);
      var yVh = lerp(fromPose.yVh, toPose.yVh, mix);
      var scale = lerp(fromPose.scale, toPose.scale, mix) * (compact ? 1.18 : 1);
      var opacity = lerp(fromPose.opacity, toPose.opacity, mix);

      stage.style.transform = 'translate3d(' + xVw.toFixed(2) + 'vw,' + yVh.toFixed(2) + 'vh,0) scale(' + scale.toFixed(4) + ')';
      stage.style.opacity = opacity.toFixed(3);

      var nearest = Math.max(0, Math.min(maxIndex, Math.round(position)));
      for (var i = 0; i < beats.length; i++) {
        var focus = Math.max(0, Math.min(1, 1 - Math.abs(position - i)));
        beats[i].style.setProperty('--story-beat-focus', focus.toFixed(3));
        beats[i].setAttribute('data-story-beat-active', i === nearest ? 'true' : 'false');
      }

      if (nearest !== activeBeat) {
        activeBeat = nearest;
        host.setAttribute('data-story-active-beat', String(nearest));
        track('cinematic_story_chapter', {
          chapter: nearest + 1,
          chapterCount: beats.length,
          sceneId: sceneId || ''
        });
      }

      // Complete the hero's internal micro-copy choreography before page beat
      // two takes over; later sections retain their own semantic DOM/content.
      applyHeroChapters(Math.min(1, progress * maxIndex));
    };
  }

  // ── Structural normalization ──────────────────────────────────────────────
  // Sticky pinning needs the stage's travel range to span the whole runway,
  // but the scaffold nests the stage inside hero grids/figures whose height
  // hugs the poster image (sticky only travels within its parent's box), and
  // the editor's image-resize can stamp inline width/height !important on the
  // stage (it doubles as the image's decorative frame). Strip the stamped
  // frame sizing and mark every wrapper between stage/chapters and the host
  // with data-story-flatten; active-mode CSS turns those into display:contents
  // so stage + chapters become direct layout children of the tall host.
  function normalizeStageStructure(host, stage){
    ['width', 'max-width', 'height', 'flex', 'align-self', 'position', 'inset',
     'left', 'right', 'top', 'bottom', 'margin', 'transform', 'opacity'].forEach(function(p){
      try { stage.style.removeProperty(p); } catch (e) {}
    });
    // The poster doubles as the static hero image, so editor zoom/reposition
    // tools may have stamped inline absolute-crop styles on it. In active
    // story mode the poster is a full-stage contain backdrop — strip the
    // stamped layout so the runtime CSS wins.
    var poster = stage.querySelector('[data-story-poster]');
    if (poster) {
      ['object-fit', 'position', 'left', 'right', 'top', 'bottom', 'width', 'height',
       'max-width', 'max-height', 'margin', 'transform'].forEach(function(p){
        try { poster.style.removeProperty(p); } catch (e) {}
      });
    }

    if (host.getAttribute('data-story-scope') === 'page') {
      var originalParent = stage.parentElement;
      if (originalParent && originalParent !== host) {
        // Preserve the hero grid's visual column after the live stage moves to
        // the corridor root. A shallow, invisible clone keeps the exact class /
        // grid-area geometry without duplicating the image or accessibility DOM.
        var slot = stage.cloneNode(false);
        slot.removeAttribute('id');
        slot.removeAttribute('data-story-stage');
        slot.setAttribute('data-story-stage-slot', '');
        slot.setAttribute('aria-hidden', 'true');
        try {
          slot.style.setProperty('visibility', 'hidden', 'important');
          slot.style.setProperty('pointer-events', 'none', 'important');
        } catch (e) {}
        originalParent.insertBefore(slot, stage);
      }
      stage.setAttribute('data-story-page-stage', '');
      var firstBeat = host.querySelector('[data-story-page-beat]');
      host.insertBefore(stage, firstBeat || host.firstChild);
      return;
    }

    var nodes = [stage];
    var chapters = host.querySelector('[data-story-chapters]');
    if (chapters) nodes.push(chapters);
    nodes.forEach(function(node){
      var el = node.parentElement;
      while (el && el !== host && el !== document.body) {
        el.setAttribute('data-story-flatten', '');
        el = el.parentElement;
      }
    });
  }

  // Themes ship 'body { overflow-x: hidden }' (horizontal-scrollbar guard),
  // which turns the body into a scroll container — and a scroll container that
  // never scrolls becomes the sticky stage's scrollport, so the stage never
  // pins. 'clip' gives the identical visual clipping WITHOUT creating a scroll
  // container. Walk host -> body and defuse every hidden-overflow ancestor.
  // (Browsers without overflow:clip ignore the assignment — same as today.)
  function neutralizeScrollAncestors(host){
  ...
  // ── Coordinator ───────────────────────────────────────────────────────────
  function initStory(){
    var host = document.querySelector('[data-zappy-cinematic-story][data-story-state="ready"]');
    if (!host || host.__zappyStoryInit) return;
    host.__zappyStoryInit = true;

    var manifest = parseManifest(host);
    if (!manifest) { host.setAttribute('data-story-tier', 'static'); return; }

    var resolution = resolveTier(manifest);
    host.setAttribute('data-story-tier', resolution.tier);
    track('cinematic_story_ready', { tier: resolution.tier, reason: resolution.reason, sceneId: manifest.sceneId });
    if (resolution.tier === 'static') return;

    host = promoteHeroStoryToPage(host);

    ensureGsap(function(err){
      if (err) {
        host.setAttribute('data-story-tier', 'static');
        track('cinematic_story_fallback', { from: resolution.tier, to: 'static', reason: 'gsap-unavailable' });
        return;
      }
      var gsap = window.gsap;
      var ScrollTrigger = window.ScrollTrigger;
      ...
      var stage = host.querySelector('[data-story-stage]');
      if (!stage) { host.setAttribute('data-story-tier', 'static'); return; }

      // Activate the runway: CSS turns the host into a tall scroll track with
      // a sticky 100vh stage — no ScrollTrigger pin, no pin-spacer conflicts.
      var runway = Math.max(120, Math.min(320, Number(manifest.runwayVh) || 220));
      host.style.setProperty('--story-runway', runway + 'vh');
      normalizeStageStructure(host, stage);
      neutralizeScrollAncestors(host);
      host.setAttribute('data-story-active', 'true');

      var label = host.getAttribute('aria-label') || host.getAttribute('data-story-label') || '';
      var adapter = null;
      var latestProgress = 0;

      function mountFrames(variant, fallbackReason){
        var set = variant === 'mobile' ? manifest.frames.mobile : manifest.frames.desktop;
        if (!set) { host.setAttribute('data-story-tier', 'static'); return null; }
        if (fallbackReason) {
          host.setAttribute('data-story-tier', 'frames');
          track('cinematic_story_fallback', { from: 'model', to: 'frames', reason: fallbackReason });
        }
        return createFrameAdapter(host, stage, set, label);
      }

      if (resolution.tier === 'model') {
        // The <model-viewer> element is inert until the library upgrades it,
        // so load the module first and fall back to frames when it cannot
        // load. adapter stays null until then; the ScrollTrigger below
        // tolerates that (onProgress is a no-op while adapter is null).
        var fallbackFromModel = function(reason){
          adapter = (manifest.renderers.allowFrames && manifest.frames)
            ? mountFrames('desktop', reason)
            : null;
          if (adapter) adapter.onProgress(latestProgress);
          if (!adapter) {
            host.setAttribute('data-story-tier', 'static');
            track('cinematic_story_fallback', { from: 'model', to: 'static', reason: reason });
          }
        };
        loadModelViewer(function(mvErr){
          if (mvErr) { fallbackFromModel('model-viewer-load-failed'); return; }
          adapter = createModelAdapter(host, stage, manifest.model, label, fallbackFromModel);
          adapter.onProgress(latestProgress);
        });
      } else {
        adapter = mountFrames(resolution.variant || 'desktop', null);
        if (!adapter) return;
      }

      track('cinematic_story_renderer_selected', { renderer: resolution.tier, sceneId: manifest.sceneId });

      var pageScope = host.getAttribute('data-story-scope') === 'page';
      var applyChapters = pageScope
        ? setupPageStory(host, stage, gsap, manifest.sceneId)
        : setupChapters(host, gsap);
      var milestones = { 25: false, 50: false, 75: false, 100: false };
      var pageBeats = pageScope
        ? Array.prototype.slice.call(host.querySelectorAll('[data-story-page-beat]'))
        : [];
      var endBeat = pageBeats.length ? pageBeats[pageBeats.length - 1] : null;
      applyChapters(0);
      if (adapter) adapter.onProgress(0);

      ScrollTrigger.create({
        trigger: host,
        endTrigger: endBeat || host,
        start: 'top top',
        end: 'bottom bottom',
        scrub: 0.4,
        invalidateOnRefresh: true,
        onEnter: function(){ host.removeAttribute('data-story-after'); },
        onEnterBack: function(){ host.removeAttribute('data-story-after'); },
        onLeave: function(){ if (pageScope) host.setAttribute('data-story-after', 'true'); },
        onUpdate: function(self){
          var p = Math.max(0, Math.min(1, self.progress));
          latestProgress = p;
          if (adapter) adapter.onProgress(p);
          applyChapters(p);
          var pct = Math.round(p * 100);
          [25, 50, 75, 100].forEach(function(m){
            if (pct >= m && !milestones[m]) {
              milestones[m] = true;
              track('cinematic_story_progress', { milestone: m, sceneId: manifest.sceneId });
            }
          });
        }
      });

      // Editor's dynamic edit mode pauses the scrub visuals so inline tools
      // can select elements without the stage moving under the cursor.
      var observer = new MutationObserver(function(){
        var editing = document.body.classList.contains('zappy-edit-mode');
        host.setAttribute('data-story-editing', editing ? 'true' : 'false');
      });
  ...
  if (reduce) {
    // Static reveal only — chapters stay visible, poster stays; no requests.
    ready(function(){
      var host = document.querySelector('[data-zappy-cinematic-story]');
      if (host) host.setAttribute('data-story-tier', 'static');
    });
    return;
  }
  ready(initStory);
})();
/* End ZAPPY_CINEMATIC_STORY_RUNTIME */


/* SIGNATURE MOMENT: scroll-end-easter-egg */
(function(){

      (function(){
        var el = document.createElement('div'); el.className = 'zsm-end'; el.textContent = 'You\u2019re all caught up'; document.body.appendChild(el); var shown = false;
        function check(){ var atEnd = (window.scrollY + window.innerHeight) >= (document.documentElement.scrollHeight - 4); if (atEnd && !shown) { shown = true; el.classList.add('is-visible'); setTimeout(function(){ el.classList.remove('is-visible'); }, 3500); } }
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '6f121752-e1a0-4fa8-bac1-12e0f1d267c2',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '6f121752-e1a0-4fa8-bac1-12e0f1d267c2';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '6f121752-e1a0-4fa8-bac1-12e0f1d267c2';
